...
function MatchCardImpl({ data, selectable = false, selected = false, onSelect, }) {
    ...
    return (_jsxs("div", { 
        ...
        className: cn("relative group/match", expanded ? "match-card-expanded" : "match-card-collapsed", selectable && "cursor-pointer", selected && "rounded-md shadow-[0_0_24px_-4px_rgba(0,217,146,0.5)]"), onClick: (e) => {
            ...
            setExpanded((prev) => !prev);
                    ...
                    : coloredMatchBg
                        ...
                        : "bg-filmdark/18 backdrop-blur-lg saturate-150 glass-panel", "shadow-[0_10px_30px_rgba(var(--c-shadow),0.55),inset_0_0_0_0.35px_rgba(255,255,255,0.05),inset_0_1px_0_rgba(255,255,255,0.025)]", selected && "ring-2 ring-jade/80"), children: [isRemake && (_jsxs(_Fragment, { children: [_jsx("div", { className: "pointer-events-none absolute inset-0 z-[1] opacity-[0.07]", style: {
...
function matchCardPropsEqual(prev, next) {
    ...
    if (prev.selected !== next.selected || prev.selectable !== next.selectable)
        return false;
    const a = prev.data;
    const b = next.data;
    if (a === b)
