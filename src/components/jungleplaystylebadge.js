...
export function JunglePlaystyleBadge({ tag, topsideCount, botsideCount }) {
    ...
    if (!label)
        return null;
    const badge = (_jsxs("div", { className: cn("h-5 flex items-center gap-1.5 pl-2 pr-2.5", "font-mono text-[9px] uppercase tracking-[0.1em]", "border-l-2 bg-filmdark/30", getJunglePlaystyleTagClasses(tag)), children: [_jsx("span", { className: "opacity-40 text-[8px] leading-none", children: "\u25C8" }), _jsx("span", { children: label })] }));
...
export function JungleStartingCampBadge({ camp }) {
    ...
    if (!label)
        return null;
    return (_jsxs("div", { className: cn("h-5 flex items-center gap-1.5 pl-2 pr-2.5", "font-mono text-[9px] uppercase tracking-[0.1em]", "border-l-2 bg-filmdark/30", getStartingCampClasses(camp)), children: [_jsx("span", { className: "opacity-40 text-[8px] leading-none", children: "\u25C8" }), _jsx("span", { children: label })] }));
}
export function JungleInvadeBadge({ invade }) {
    const label = getReadableInvade(invade);
    if (!label)
        return null;
    return (_jsxs("div", { className: cn("h-5 flex items-center gap-1.5 pl-2 pr-2.5", "font-mono text-[9px] uppercase tracking-[0.1em]", "border-l-2 bg-filmdark/30", getInvadeClasses(invade)), children: [_jsx("span", { className: "opacity-40 text-[8px] leading-none", children: "\u25C8" }), _jsx("span", { children: label })] }));
}
