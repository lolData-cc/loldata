...
export const CHAMP_CLASSES = ["Assassin", "Fighter", "Mage", "Marksman", "Support", "Tank"];
...
export const CATEGORIES = [...CHAMP_CLASSES, "AD", "AP", "Melee", "Ranged"];
...
export const categoryHasIcon = (cls) => CHAMP_CLASSES.includes(cls);
...
export const categoryIcon = (cls) => `https://cdn2.loldata.cc/img/class/${cls.toLowerCase()}.png`;
...
export const champIcon = (name) => `${cdnBaseUrl()}/img/champion/${normalizeChampName(name)}.png`;
export const itemIcon = (id) => `${cdnBaseUrl()}/img/item/${id}.png`;
// live name (from item.json) first, then the static fallback, then a bare id
export const itemName = (id) => itemDisplayName(id) ?? ITEMS.find((i) => i.id === id)?.name ?? `Item ${id}`;
