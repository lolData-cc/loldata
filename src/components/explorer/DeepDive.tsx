...
import { X, TrendingUp, TrendingDown, Minus as MinusIcon, ChevronRight } from "lucide-react";
import {
...
export function DeepDive({ data, graph, onClose }: { data: QueryResult; graph: ExplorerGraph; onClose: () => void }) {
  ...
  return (
    <div
      className="absolute inset-0 z-[20] overflow-y-auto cyber-scrollbar bg-[rgba(3,7,9,0.975)] backdrop-blur-2xl"
      style={{ animation: "deepDiveExpand 0.5s cubic-bezier(0.16,1,0.3,1)" }}
    >
      {/* ambient glows */}
      ...
      <div className="relative max-w-[1080px] mx-auto px-6 md:px-10 py-7">
        {/* header */}
        <div className="deep-section flex items-start justify-between gap-4 pb-5 mb-6 border-b border-white/[0.07]">
          <div className="flex items-center gap-3.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-[13px] font-chakrapetch font-bold tabular-nums text-flash">{data.meta.games.toLocaleString()}</span>
                <span className="text-[8px] font-chakrapetch uppercase tracking-[0.18em] text-flash/40 -ml-0.5">games</span>
                <span className="px-2 py-0.5 rounded-[4px] bg-jade/10 border border-jade/30 text-[8px] font-chakrapetch font-bold uppercase tracking-[0.12em] text-jade">
                  {data.meta.mode === "all" ? "all patches" : data.meta.patch ? `patch ${data.meta.patch}` : "current patch"}
                </span>
                {rank && <span className="px-2 py-0.5 rounded-[4px] bg-citrine/10 border border-citrine/30 text-[8px] font-chakrapetch font-bold uppercase tracking-[0.12em] text-citrine">{rank}</span>}
              </div>
            </div>
          </div>
          <button onClick={onClose} title="Collapse (Esc)" className="shrink-0 grid place-items-center w-9 h-9 rounded-[6px] border border-white/10 text-flash/45 hover:text-flash hover:border-white/25 hover:bg-white/[0.05] transition-colors cursor-clicker">
            <X size={17} />
          </button>
        </div>
        ...
        {/* PATCH VARIATION — the centrepiece */}
        ...
        <p className="deep-section mt-8 text-[10px] font-chakrapetch text-flash/25 leading-relaxed" style={{ animationDelay: "180ms" }}>
          {isItemDim
            ? "Item value weights each item's winrate lift over baseline by sample size, so a proven core build outranks a high-variance niche pick. Build path + matchup analysis use timeline build-order data."
