...
function PatchVariation({ graph }) {
    ...
    if (phase === "loading") {
        return (_jsxs("div", { className: "mt-1 flex flex-col gap-2 pt-3 border-t border-hairline/[0.06]", children: [header, _jsxs("div", { className: "flex items-center gap-2 text-flash/40 text-[10px] font-chakrapetch py-1.5", children: [_jsx(Loader2, { size: 12, className: "animate-spin" }), " loading patch trend\u2026"] })] }));
    }
    if (phase !== "ready" || !rows) {
        return (_jsxs("div", { className: "mt-1 flex flex-col gap-2 pt-3 border-t border-hairline/[0.06]", children: [header, _jsx("span", { className: "text-[10px] font-chakrapetch text-flash/35 py-0.5", children: phase === "error" ? "Patch trend unavailable." : "Not enough patches to chart a trend." })] }));
    }
    ...
    return (_jsxs("div", { className: "mt-1 flex flex-col gap-2.5 pt-3 border-t border-hairline/[0.06]", children: [header, _jsx("div", { className: "flex gap-1", children: PV_METRICS.map((x) => (_jsx("button", { onClick: () => setMetric(x.key), className: cn("flex-1 py-1 rounded-[4px] text-[9px] font-chakrapetch font-bold tracking-[0.1em] uppercase transition-colors cursor-clicker", metric === x.key ? "bg-jade/15 text-jade border border-jade/35" : "text-flash/40 border border-hairline/[0.07] hover:text-flash/70"), children: x.label }, x.key))) }), _jsx("div", { className: "flex flex-col gap-1", children: rows.map((r, i) => {
                    const v = m.get(r);
