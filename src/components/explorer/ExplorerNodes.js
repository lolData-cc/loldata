...
const numCls = "nodrag w-full h-8 bg-filmdark/40 border border-jade/25 rounded-[4px] px-2 " +
...
export function ExplorerNode({ id, type, data, selected }) {
    ...
    return (_jsxs("div", { className: cn("group relative w-[224px] rounded-[7px] border bg-[rgba(8,14,16,0.86)] backdrop-blur-md transition-colors", selected ? "border-transparent" : excluded ? "border-error/45" : "border-hairline/10", excluded && "rounded-b-none border-b-error/30", // bottom edge mates with the EXCLUDE piece keyed beneath
