...
export async function runQuery(graph) {
...
export async function runPatchVariation(graph, exact = false) {
    if (exact)
        await ensurePools(graph);
...
export async function runBuildPath(graph) {
    await loadItems().catch(() => { });
    const g = { ...graph, itemPool: graph.itemPool?.length ? graph.itemPool : getSlotPoolIds() };
...
export async function runItemStrength(graph, itemId) {
...
export function compileGraph(nodes, edges) {
    ...
    const inputsOf = (id) => edges.filter((e) => e.target === id).map((e) => byId.get(e.source)).filter((n) => !!n);
    // A module with its EXCLUDE submodule on (data.exclude) becomes a NEGATIVE
    // constraint — "winrate WITHOUT this".
    const isExcluded = (n) => n.data.exclude === true;
    // Item / Rune modules wired INTO a champion (or the Output) node.
    const attach = (id) => {
        const a = inputsOf(id);
        const items = [], excludeItems = [];
        const itemSlots = {}; // 1-6 = Nth completed item; 0/undefined = any
        for (const n of a.filter((n) => n.type === "item" && n.data.itemId)) {
            const itemId = n.data.itemId;
            if (isExcluded(n)) {
                excludeItems.push(itemId);
                continue;
            } // excluded → no slot
            items.push(itemId);
            const s = Number(n.data.slot) | 0;
            if (s >= 1 && s <= 6)
                itemSlots[itemId] = s;
        }
        const keystones = [], excludeKeystones = [];
        for (const n of a.filter((n) => n.type === "rune" && n.data.keystone)) {
            (isExcluded(n) ? excludeKeystones : keystones).push(n.data.keystone);
        }
        return { items, excludeItems, keystones, excludeKeystones, itemSlots };
    };
    const output = nodes.find((n) => n.type === "output");
    if (!output)
        return { error: "Add an Output node" };
    const outInputs = inputsOf(output.id);
    const subjectNode = outInputs.find((n) => n.type === "subject");
    if (!subjectNode)
        return { error: "Connect a Subject → Output" };
    const sd = subjectNode.data;
    if (!sd.champion)
        return { error: "Pick a champion on the Subject" };
    ...
    const constraintNodes = [...outInputs, ...inputsOf(subjectNode.id)].filter((n, i, arr) => arr.findIndex((m) => m.id === n.id) === i);
    ...
    const isDefaultQueues = (q) => q.length === DEFAULT_QUEUES.length && [...q].map(Number).sort().join() === DEFAULT_QUEUES.join();
    ...
    if (filterNodes.length) {
        const merged = { tiers: [], queues: DEFAULT_QUEUES, platforms: [] };
        for (const fn of filterNodes) {
            const d = fn.data;
            if (d.scope)
                merged.scope = d.scope; // last scope wins
            if (d.tiers?.length)
                merged.tiers = d.tiers; // last explicit tier set wins (empty = "all ranks" = no-op)
            if (d.queues?.length && !isDefaultQueues(d.queues))
                merged.queues = d.queues; // last *narrowed* queue wins over the Solo+Flex default
            if (d.platforms?.length)
                merged.platforms = d.platforms; // last explicit region set wins (empty = all regions = no-op)
        }
    ...
    if (slotTargets(graph).length)
        graph.itemPool = slotPool(graph);
    return { graph };
}
// the item ids that carry a build-slot constraint (subject + constraints)
function slotTargets(graph) {
    return [
...
async function ensurePools(graph) {
    const needsSlot = slotTargets(graph).length > 0;
    const needsRankItem = graph.output.kind === "rank" && graph.output.dimension === "item";
    if (!needsSlot && !needsRankItem)
        return;
    await loadItems().catch(() => { });
    if (needsSlot)
        graph.itemPool = slotPool(graph);
    if (needsRankItem && graph.output.kind === "rank")
        graph.output.itemPool = getLoadedItemIds();
}
...
export function mockRun(graph) {
    const key = JSON.stringify(graph);
    const constraintTightness = (graph.subject.items?.length ?? 0) +
    ...
    if (graph.output.kind === "rank") {
        const pool = graph.output.dimension === "item"
            ? ITEMS.map((i) => i.name)
            : CHAMPIONS;
