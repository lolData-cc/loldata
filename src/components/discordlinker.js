...
export function DiscordLinker() {
        ...
        (async () => {
            try {
                ...
                const { data, error } = await supabase
                    ...
                    .select("profile_id, player_id, discord_id, discord_username, discord_avatar_url, discord_public")
    ...
    const setDiscordVisibility = useCallback(async (next) => {
        if (!profile || savingVisibility)
            return;
    ...
    // ===== UI =====
    const statusHint = loadingState ? (_jsx("span", { className: "text-[9px] uppercase tracking-[0.2em] text-flash/30 font-mono", children: "LOADING\u2026" })) : isLinked ? (_jsx("span", { className: "text-[9px] uppercase tracking-[0.2em] text-jade/60 font-mono", children: "\u25C8 CONNECTED" })) : discordFromIdentity ? (_jsx("span", { className: "text-[9px] uppercase tracking-[0.2em] text-citrine/60 font-mono", children: "READY TO LINK" })) : (_jsx("span", { className: "text-[9px] uppercase tracking-[0.2em] text-flash/30 font-mono", children: "NOT CONNECTED" }));
    return (_jsx(SettingsCard, { title: "Discord", hint: statusHint, children: _jsxs("div", { className: "flex items-center gap-3.5", children: [_jsx("div", { className: "h-14 w-14 shrink-0 overflow-hidden rounded-[2px] border border-jade/15 bg-filmdark/30", children: isLinked && effectiveDiscord?.avatarUrl && !avatarBroken ? (_jsx("img", { src: effectiveDiscord.avatarUrl, alt: "Discord avatar", onError: () => setAvatarBroken(true), className: "h-full w-full object-cover" })) : (_jsx("div", { className: "grid h-full w-full place-items-center text-[9px] font-mono text-flash/30", children: "N/A" })) }), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("div", { className: "truncate text-sm font-medium text-flash/85", children: isLinked
                                ? (effectiveDiscord?.username ?? "Discord user")
                                : discordFromIdentity
                                    ? "Ready to link"
                                    : "Not connected" }), _jsx("div", { className: "mt-0.5 line-clamp-2 text-[12px] leading-snug text-flash/40", children: isLinked ? (_jsxs("span", { className: "inline-flex items-center gap-2", children: ["Show on public profile", _jsx(CyberToggle, { checked: discordPublic, onChange: setDiscordVisibility })] })) : ("Connect your Discord to unlock lolData integrations.") })] }), _jsx("div", { className: "flex shrink-0 items-center gap-2", children: !loadingState && (_jsx(_Fragment, { children: isLinked ? (_jsx("button", { type: "button", onClick: handleUnlinkDiscord, disabled: disconnecting, className: "rounded-[2px] border border-flash/15 px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.1em] text-flash/50 transition-colors hover:bg-flash/5 cursor-clicker disabled:opacity-40 disabled:pointer-events-none", children: disconnecting ? "UNLINKING…" : "UNLINK" })) : discordFromIdentity ? (_jsx("button", { type: "button", onClick: handleLinkDiscord, disabled: linking, className: "inline-flex items-center gap-1.5 rounded-[2px] border border-jade/35 bg-jade/10 px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.1em] text-jade transition-colors hover:bg-jade/20 hover:border-jade/50 cursor-clicker disabled:opacity-50 disabled:pointer-events-none", children: linking ? "LINKING…" : "LINK" })) : (_jsx("button", { type: "button", onClick: handleConnectDiscordOAuth, disabled: connectingOAuth, className: "inline-flex items-center gap-1.5 rounded-[2px] border border-jade/35 bg-jade/10 px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.1em] text-jade transition-colors hover:bg-jade/20 hover:border-jade/50 cursor-clicker disabled:opacity-50 disabled:pointer-events-none", children: connectingOAuth ? "CONNECTING…" : "CONNECT" })) })) })] }) }));
}
