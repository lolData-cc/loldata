// src/components/matchreplay/DrawingOverlay.tsx
//
// Whiteboard-style drawing surface that sits on top of the map.
//
// Modes:
//   "off"     → canvas is pointer-events: none; everything passes
//               through to map/sprites (default — no interference).
//   "pen"     → draw freehand strokes; stored normalized 0..1 so they
//               survive resize. Multiple colors available.
//   "eraser"  → wide stroke in composite "destination-out" — wipes
//               any drawn paths it touches.
//
// Strokes persist in the parent's state (so they survive view-tab
// switches / playback toggles). "Clear" wipes them all.
//
// Toolbar lives floating top-left of the map and toggles between
// modes / colors / clear / undo.
...
export type DrawTool = "off" | "pen" | "eraser";

export interface Stroke {
  /** "eraser" or a CSS color string */
  kind: "pen" | "eraser";
  color: string;
  /** stroke width as a fraction of the canvas SHORT side (0.005 ~ 1px on 200px canvas) */
  width: number;
  /** points are normalized 0..1 against the canvas */
...
export function DrawingOverlay({ tool, color, strokes, setStrokes }: DrawingOverlayProps) {
...
export function DrawingToolbar({ tool, setTool, color, setColor, onClear, onUndo, hasStrokes }: DrawingToolbarProps) {
  return (
        ...
        <div className="flex flex-col gap-0.5 p-1 rounded-sm bg-liquirice/80 backdrop-blur-md ring-1 ring-flash/10 shadow-[0_4px_14px_rgba(0,0,0,0.55)]">
          {DRAW_COLORS.map((c) => (
            <button
              ...
              onClick={() => setColor(c)}
              className={cn(
                "w-5 h-5 rounded-sm cursor-clicker transition-all",
                color === c ? "ring-2 ring-flash/70 scale-110" : "ring-1 ring-flash/20 hover:scale-105"
              )}
              style={{ background: c }}
              title={c}
            />
...
function ToolBtn({
  ...
  return (
    <button
      ...
      title={label}
      className={cn(
        "p-1.5 rounded-sm transition-all cursor-clicker flex items-center justify-center",
        active
          ? "text-jade bg-jade/15 ring-1 ring-jade/40"
          : danger
            ? "text-flash/55 hover:text-[#d63336] hover:bg-[#d63336]/10"
            : "text-flash/55 hover:text-jade hover:bg-jade/10",
      )}
    >
      {icon}
    </button>
