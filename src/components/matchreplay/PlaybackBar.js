...
export function PlaybackBar({ timeline, durationMs, timeMs, onSeek, }) {
    ...
    return (_jsxs("div", { className: "w-full select-none", children: [_jsx("div", { className: "relative h-3 mb-0.5", children: markers.map((m, i) => {
                    ...
                    return (_jsx("button", { type: "button", onClick: () => onSeek(m.t), title: m.label, className: "absolute top-1/2 -translate-y-1/2 cursor-pointer focus:outline-none", style: { left: `${left}%`, transform: "translate(-50%, -50%)" }, children: _jsx("div", { className: cn("w-[5px] h-[5px] rounded-sm transition-all hover:scale-[1.6]", m.kind === "kill" ? "rotate-45" : ""), style: { background: m.tint, boxShadow: `0 0 3px ${m.tint}` } }) }, i));
                        ...
                        }) }), _jsxs("div", { className: "absolute right-0 bottom-0 flex items-baseline gap-1 font-mono tabular-nums leading-none bg-liquirice/80 px-1 rounded-sm", children: [_jsx("span", { className: "text-jade text-[11px] font-semibold", children: fmtClock(timeMs) }), _jsx("span", { className: "text-flash/25 text-[9px]", children: "/" }), _jsx("span", { className: "text-flash/45 text-[9px]", children: fmtClock(durationMs) })] })] })] }));
}
