...
export const BLUE_IDS: number[] = [1, 2, 3, 4, 5];
export const RED_IDS: number[] = [6, 7, 8, 9, 10];

export function teamOf(pid: number): TeamId {
 ...
 */
export function puuidToPid(tl: MatchTimeline): Map<string, number> {
 ...
 */
export function frameAt(tl: MatchTimeline, timeMs: number): { lo: number; hi: number; t: number } {
  const frames = tl.info.frames;
  if (!frames.length) return { lo: 0, hi: 0, t: 0 };
  if (timeMs <= frames[0].timestamp) return { lo: 0, hi: 0, t: 0 };
  const last = frames.length - 1;
  if (timeMs >= frames[last].timestamp) return { lo: last, hi: last, t: 0 };

  // Binary search.
  let lo = 0, hi = last;
  while (lo + 1 < hi) {
    const mid = (lo + hi) >>> 1;
    if (frames[mid].timestamp <= timeMs) lo = mid;
    else hi = mid;
  }
 ...
 */
export function positionsAt(tl: MatchTimeline, timeMs: number): Map<number, Position> {
  ...
  for (let pid = 1; pid <= 10; pid++) {
    ...
    const pb = b?.position;
    if (!pa && !pb) continue;
    if (!pa) { out.set(pid, pb!); continue; }
    if (!pb) { out.set(pid, pa); continue; }
    out.set(pid, {
      x: pa.x + (pb.x - pa.x) * t,
      y: pa.y + (pb.y - pa.y) * t,
    });
 ...
 */
export function metricsAt(tl: MatchTimeline, pid: number, timeMs: number): {
  level: number;
  xp: number;
  currentGold: number;
  totalGold: number;
  cs: number;
  jungleCs: number;
  damageStats: ParticipantFrame["damageStats"];
  championStats: ParticipantFrame["championStats"];
  ...
  const A = a ?? b!;
  const B = b ?? a!;
  const lerpNum = (x: number | undefined, y: number | undefined) => {
    const xv = x ?? 0; const yv = y ?? 0;
    return xv + (yv - xv) * t;
  };
  ...
  const lerpDict = (
    aDict: Record<string, number | undefined> | undefined,
    bDict: Record<string, number | undefined> | undefined,
  ...
  return {
    ...
    damageStats: lerpDict(A.damageStats as Record<string, number | undefined>, B.damageStats as Record<string, number | undefined>) as ParticipantFrame["damageStats"],
    championStats: lerpDict(A.championStats as Record<string, number | undefined>, B.championStats as Record<string, number | undefined>) as ParticipantFrame["championStats"],
 ...
 */
export function eventsUpTo(tl: MatchTimeline, timeMs: number): Array<TimelineEvent & { _frame: number }> {
  const out: Array<TimelineEvent & { _frame: number }> = [];
  for (let i = 0; i < tl.info.frames.length; i++) {
    const f = tl.info.frames[i];
    for (const e of f.events) {
      if (e.timestamp > timeMs) return out;
      out.push({ ...e, _frame: i });
    }
  }
  return out;
 ...
 */
export function allEvents(tl: MatchTimeline): Array<TimelineEvent & { _frame: number }> {
  const out: Array<TimelineEvent & { _frame: number }> = [];
  for (let i = 0; i < tl.info.frames.length; i++) {
    for (const e of tl.info.frames[i].events) out.push({ ...e, _frame: i });
  }
  return out;
}
 ...
 */
export function durationMs(tl: MatchTimeline): number {
...
export function inventoryAt(tl: MatchTimeline, pid: number, timeMs: number): {
  items: number[];
  trinketId: number | null;
} {
  const counts = new Map<number, number>();
  const bump = (id: number | undefined, d: number) => {
    if (!id) return;
  ...
  for (const f of tl.info.frames) {
    for (const e of f.events) {
      ...
      if (e.type === "ITEM_PURCHASED") bump(e.itemId, +1);
      else if (e.type === "ITEM_DESTROYED" || e.type === "ITEM_SOLD") bump(e.itemId, -1);
      else if (e.type === "ITEM_UNDO") {
        if (e.beforeId) bump(e.beforeId, -1);
        if (e.afterId) bump(e.afterId, +1);
      }
    }
  }
  return finalize();
  ...
  function finalize() {
    ...
    for (const [id, n] of counts) {
      for (let k = 0; k < n; k++) {
        if (TRINKET_IDS.has(id)) trinketId = id;
        else items.push(id);
      }
    }
 ...
 */
export function kdaAt(tl: MatchTimeline, timeMs: number): Map<number, { k: number; d: number; a: number }> {
  const out = new Map<number, { k: number; d: number; a: number }>();
  for (let i = 1; i <= 10; i++) out.set(i, { k: 0, d: 0, a: 0 });
  for (const f of tl.info.frames) {
    for (const e of f.events) {
      if (e.timestamp > timeMs) return out;
      ...
      if (killer && killer > 0) out.get(killer)!.k += 1;
      if (victim && victim > 0) out.get(victim)!.d += 1;
      for (const aid of e.assistingParticipantIds ?? []) {
        if (aid > 0) out.get(aid)!.a += 1;
      }
    }
  }
  return out;
 ...
 */
export function teamGoldAt(tl: MatchTimeline, timeMs: number): { blue: number; red: number; diff: number } {
  let blue = 0, red = 0;
  for (let pid = 1; pid <= 10; pid++) {
    const m = metricsAt(tl, pid, timeMs);
    if (!m) continue;
    if (teamOf(pid) === 100) blue += m.totalGold;
    else red += m.totalGold;
  }
 ...
 */
export function goldDiffSeries(tl: MatchTimeline): Array<{ t: number; blue: number; red: number; diff: number }> {
  const out: Array<{ t: number; blue: number; red: number; diff: number }> = [];
  for (const f of tl.info.frames) {
    let blue = 0, red = 0;
    for (let pid = 1; pid <= 10; pid++) {
      const pf = f.participantFrames[String(pid)];
      if (!pf) continue;
      if (teamOf(pid) === 100) blue += pf.totalGold ?? 0;
      else red += pf.totalGold ?? 0;
    }
 ...
 */
export function damageRankedAt(tl: MatchTimeline, timeMs: number): Array<{ pid: number; dmg: number }> {
  const out: Array<{ pid: number; dmg: number }> = [];
  for (let pid = 1; pid <= 10; pid++) {
    const m = metricsAt(tl, pid, timeMs);
    if (!m) continue;
 ...
 */
const WARD_LIFESPAN_MS: Record<string, number> = {
...
export function activeWardsAt(tl: MatchTimeline, timeMs: number, positionAtPlacement: (creatorId: number, ts: number) => Position | null): ActiveWard[] {
  const placed = new Map<string, ActiveWard>();
  let serial = 0;
  for (const f of tl.info.frames) {
    for (const e of f.events) {
      if (e.timestamp > timeMs) return Array.from(placed.values());
      if (e.type === "WARD_PLACED" && e.creatorId) {
        ...
        for (const [k, w] of Array.from(placed).reverse()) {
          if (w.wardType === e.wardType) { placed.delete(k); break; }
        }
      }
    }
  }
  ...
  for (const [k, w] of placed) {
    ...
    if (ttl > 0 && timeMs - w.placedAt > ttl) placed.delete(k);
  }
  return Array.from(placed.values());
}

/**
 * Latest objective state at time T — handy for the top header strip.
 * We track:
 *   - dragons killed per team (with subType list)
 *   - barons per team
 *   - heralds per team
 *   - voidgrubs per team
 *   - towers killed per team
 *   - inhibitors killed per team
 */
export interface ObjectiveTally {
  dragons: Array<string>;   // monsterSubType in chrono order
  barons: number;
  heralds: number;
  voidgrubs: number;
  atakhans: number;
...
export function objectivesAt(tl: MatchTimeline, timeMs: number): { blue: ObjectiveTally; red: ObjectiveTally } {
  const blank = (): ObjectiveTally => ({ dragons: [], barons: 0, heralds: 0, voidgrubs: 0, atakhans: 0, towers: 0, inhibitors: 0 });
  const blue = blank(), red = blank();
  for (const f of tl.info.frames) {
    for (const e of f.events) {
      if (e.timestamp > timeMs) return { blue, red };
      if (e.type === "ELITE_MONSTER_KILL") {
        const t = e.killerTeamId === 100 ? blue : e.killerTeamId === 200 ? red : null;
        if (!t) continue;
        if (e.monsterType === "DRAGON") t.dragons.push(String(e.monsterSubType ?? "DRAGON"));
        else if (e.monsterType === "BARON_NASHOR") t.barons++;
        else if (e.monsterType === "RIFTHERALD") t.heralds++;
        else if (e.monsterType === "HORDE") t.voidgrubs++;
        else if (e.monsterType === "ATAKHAN") t.atakhans++;
        ...
        if (!t) continue;
        if (e.buildingType === "TOWER_BUILDING") t.towers++;
        else if (e.buildingType === "INHIBITOR_BUILDING") t.inhibitors++;
      }
    }
  }
  return { blue, red };
 ...
 */
export function pidForPuuid(tl: MatchTimeline, puuid: string): number | null {
 ...
 */
export function toMapNorm(p: Position): { nx: number; ny: number } {
 ...
 */
export function staticParticipantByPid(m: StaticMatch | null | undefined, pid: number): StaticParticipant | null {
  if (!m) return null;
  for (let i = 0; i < m.info.participants.length; i++) {
    const sp = m.info.participants[i];
    const rowPid = sp.participantId ?? (i + 1);
    if (rowPid === pid) return sp;
  }
  return null;
}
