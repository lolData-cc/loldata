...
const WIPE_IN = [0.16, 1, 0.3, 1] as const;
const WIPE_OUT = [0.7, 0, 0.84, 0] as const;
...
export function DesktopShowcase({ id }: { id?: string }) {
  return (
    <Showcase id={id} mock={<DesktopMock />}>
      <Eyebrow>Desktop app · in development</Eyebrow>
      <Headline>
        A second screen that <Hot>thinks ahead</Hot>.
      </Headline>
      <Lead>
        It sits over the game and reads it as it happens — who you are against,
        what they built, where the jungle actually is. Then it tells you the next
        thing to do: the clear that fits this matchup, the item that wins against
        this composition, the reset you are about to miss. No tab out, no
        guessing.
      </Lead>
      <motion.div variants={up} className="pt-3">
        <DesktopCta />
      </motion.div>
    </Showcase>
  );
...
function DesktopMock() {
  return (
    <div className="relative mx-auto w-full max-w-[560px]" style={{ perspective: "1500px" }}>
      {/* Spotlight thrown from above-left, so the tilt has a reason to exist.
          A radial gradient only fades to nothing INSIDE its element if
          centre ± radius stays within 0-100% on both axes. This was centred at
          32% 8% with a 58% 46% radius, so it reached -26% horizontally: it
          still had colour where the div ended, and the div's rectangle was
          visible as a hard edge across the glow. Both gradients below are
          contained, and the blur covers any rounding. */}
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-x-24 -top-32 -bottom-20 -z-10 blur-[18px]"
        style={{
          background:
      ...
      <motion.div
        ...
        style={{ transformStyle: "preserve-3d", rotateX: 7, rotateZ: -1 }}
        initial={{ rotateY: -18 }}
        animate={{ rotateY: [-18, -13.5, -18], y: [0, -9, 0] }}
        transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
        className="relative"
      >
        {/* the right-hand edge — what sells it as a panel with thickness */}
        <span
          aria-hidden
          className="absolute inset-y-3 -right-[6px] w-[6px] rounded-r-[4px]"
          style={{
      ...
      {/* The pool of light it stands in. Was anchored AT 50% 0% with a 100%
          vertical radius — full brightness exactly on its own top edge, which
          is a straight line of light across the page. Centred and contained
          now, same rule as above. */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 -bottom-14 h-32 -z-10 blur-[14px]"
        style={{
          background:
...
function GameScreen() {
  ...
  return (
    <div
      className="relative aspect-[16/9] overflow-hidden rounded-[10px] border border-jade/20 bg-[#050b0d]"
      style={{
        boxShadow:
          "0 60px 120px -50px rgba(0,217,146,0.35), 0 0 0 1px rgba(0,217,146,0.06), inset 0 1px 0 rgba(255,255,255,0.04)",
      }}
    >
      <img
        src={INGAME_SHOT}
        alt="A League of Legends game with the lolData overlay running on top of it"
        className="absolute inset-0 h-full w-full object-cover"
        loading="lazy"
        decoding="async"
      />
      {/* ── the overlay ──────────────────────────────────────────────────
          Everything the callout is made of lives inside ONE presence child, so
          it arrives and leaves as a single event. There is nothing to keep in
          sync because there is only one thing moving. */}
      <AnimatePresence>
        {showCallout && (
          <motion.div
            key="callout"
            className="absolute right-0 top-[12%] w-[43%] max-w-[228px]"
          ...
          >
            {/* The shadow. Local, so it travels with the callout, but its alpha
                reaches zero at ~62% of the radius — well inside its own box —
                and a blur on top guarantees there is no edge left to notice. A
                wash that still has alpha where its element ends is what shows a
                seam. Sits OUTSIDE the clip's own bounds visually, but inside the
                element, so it wipes away with everything else. */}
            <span
              aria-hidden
              className="pointer-events-none absolute -inset-x-8 -inset-y-7 blur-[7px]"
              style={{
                background:
            ...
            {/* the rail the reading arrives on */}
            <svg
              aria-hidden
              viewBox="0 0 228 10"
            ...
            >
              {/* chamfer down at the left end, then straight out through the edge */}
              <path
                d="M 2 9 L 9 2 L 228 2"
                fill="none"
                stroke="#00d992"
                strokeWidth="1"
                ...
                opacity="0.9"
              />
              <rect x="176" y="-2" width="7" height="7" transform="rotate(45 179.5 1.5)" fill="#00d992" />
            </svg>

            <div className="relative overflow-hidden pl-3 pr-1 pt-[9px]">
              <div className="flex items-center gap-2">
                <div
                  className="min-w-0 flex-1"
                  style={{ textShadow: "0 1px 4px rgba(0,0,0,0.95), 0 0 12px rgba(0,0,0,0.8)" }}
                >
                  <p className="font-jetbrains text-[6px] uppercase tracking-[0.3em] text-jade/85">
                    Suggested buy
                  </p>
                  <p className="font-chakrapetch text-[11.5px] font-bold leading-[1.2] text-flash">
                    Dark Seal <span className="text-jade/70">+</span> Fated Ashes
                  </p>
                  <p className="font-jetbrains text-[6.5px] leading-tight text-flash/55">
                    87% on a 3-camp reset · <span className="text-citrine/80">1250g</span>
                  </p>
                </div>
      ...
      {/* screen sheen, last so it sits over everything */}
      <span
        aria-hidden
        className="pointer-events-none absolute inset-0 z-20"
        style={{
          background:
            "linear-gradient(112deg, rgba(255,255,255,0.05) 0%, transparent 32%, transparent 70%, rgba(0,217,146,0.045) 100%)",
        }}
      />
    </div>
  )
