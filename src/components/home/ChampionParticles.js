...
async function sampleImage(src, opts) {
...
function Cloud({ src }) {
    ...
    if (!geo)
        return null;
    return (_jsxs("points", { ref: ref, children: [_jsxs("bufferGeometry", { children: [_jsx("bufferAttribute", { attach: "attributes-position", args: [geo.positions, 3] }), _jsx("bufferAttribute", { attach: "attributes-color", args: [geo.colors, 3] })] }), _jsx("pointsMaterial", { size: 0.05, sizeAttenuation: true, vertexColors: true, transparent: true, opacity: 0.95, depthWrite: false, blending: THREE.AdditiveBlending })] }));
}
export function ChampionParticles({ src = "/img/Yasuo.png", className }) {
    ...
    if (mode === "flat") {
        return (_jsx("img", { src: src, alt: "", "aria-hidden": true, draggable: false, className: cn("h-full w-full object-cover object-top opacity-45", className) }));
    }
    if (mode === "loading")
        return _jsx("div", { className: className });
    return (_jsx("div", { className: className, children: _jsx(Canvas, { dpr: [1, 1.6], camera: { position: [0, 0, 12], fov: 45 }, gl: { antialias: true, alpha: true, powerPreference: "high-performance" }, style: { background: "transparent" }, children: _jsx(Cloud, { src: src }) }) }));
}
