...
function BuildPanel({ participantId, timeline, scrollRef }) {
    ...
    return (_jsxs("div", { className: "flex-1 flex flex-col min-h-0", children: [_jsx("div", { className: "border border-cyan-500/[0.07] rounded-md bg-[#08080a] shadow-[inset_0_0_20px_rgba(var(--c-shadow),0.4)] flex-1 flex items-center", children: _jsx("div", { ref: scrollRef, className: "flex gap-4 overflow-x-auto scrollbar-hide px-6 py-3 w-full items-center", children: groupedEvents.map((group, idx) => {
                        const boughtCounts = getItemCounts(group.bought);
                        return (_jsxs(_Fragment, { children: [idx > 0 && (_jsx("span", { className: "flex items-center shrink-0 text-[8px] text-cyan-500/15 select-none", children: "\u25C8" })), _jsxs("div", { className: "flex flex-col items-center min-w-[48px] shrink-0", children: [_jsxs("div", { className: "flex gap-1.5 mb-1 justify-center", children: [Object.entries(boughtCounts).map(([id, count]) => (_jsxs("div", { className: "relative w-7 h-7", children: [_jsx("img", { src: `${cdnBaseUrl()}/img/item/${id}.png`, className: "w-7 h-7 rounded ring-1 ring-hairline/10" }), count > 1 && (_jsxs("span", { className: "absolute -bottom-0.5 -right-0.5 text-[8px] bg-black/90 text-flash/60 rounded px-0.5 leading-none font-mono", children: ["x", count] }))] }, `buy-${id}`))), group.sold.map((id, i) => (_jsxs("div", { className: "relative w-7 h-7 opacity-40", children: [_jsx("img", { src: `${cdnBaseUrl()}/img/item/${id}.png`, className: "w-7 h-7 rounded grayscale ring-1 ring-hairline/5" }), _jsx("span", { className: "absolute -bottom-0.5 -right-0.5 text-[7px] bg-rose-900/80 text-rose-300 rounded px-0.5 leading-none font-mono", children: "X" })] }, `sold-${id}-${i}`)))] }), _jsx("span", { className: "text-[9px] text-flash/20 font-mono tracking-[0.15em]", children: formatGameTime(group.timestamp) })] }, idx)] }));
...
export function KillMap({ timeline, participants, focusedPlayerPuuid }) {
    ...
    function renderTooltipContent(event) {
        if (event.type === "CHAMPION_KILL") {
            ...
            return (_jsxs("div", { className: "flex items-center gap-2", children: [killer && _jsx("img", { src: `${cdnBaseUrl()}/img/champion/${killer.championName}.png`, className: "w-5 h-5 rounded-sm ring-1 ring-hairline/10" }), _jsx("span", { className: isBlueKill ? "text-cyan-300" : "text-rose-300", children: killer?.riotIdGameName ?? "Unknown" }), _jsx("span", { className: "text-flash/20", children: "///" }), _jsx("span", { className: !isBlueKill ? "text-cyan-300" : "text-rose-300", children: victim?.riotIdGameName ?? "Unknown" }), victim && _jsx("img", { src: `${cdnBaseUrl()}/img/champion/${victim.championName}.png`, className: "w-5 h-5 rounded-sm ring-1 ring-hairline/10" })] }));
        }
        if (event.type === "ELITE_MONSTER_KILL") {
            const killer = participantMap.get(event.killerId);
            const isBlue = event.killerTeamId === 100;
            const monsterName = getMonsterDisplayName(event.monsterType, event.monsterSubType);
            return (_jsxs("div", { className: "flex items-center gap-2", children: [killer && _jsx("img", { src: `${cdnBaseUrl()}/img/champion/${killer.championName}.png`, className: "w-5 h-5 rounded-sm ring-1 ring-hairline/10" }), _jsx("span", { className: isBlue ? "text-cyan-300" : "text-rose-300", children: killer?.riotIdGameName ?? (isBlue ? "Blue" : "Red") }), _jsx("span", { className: "text-flash/20", children: "///" }), _jsx("span", { className: getMonsterColor(event.monsterType), children: monsterName })] }));
        }
        const destroyerIsBlue = event.teamId === 200;
    ...
    function renderKillRow(event) {
        ...
        return (_jsxs("div", { className: "flex items-center gap-2 min-w-0", children: [killer && _jsx("img", { src: `${cdnBaseUrl()}/img/champion/${killer.championName}.png`, className: "w-4 h-4 rounded-[3px] shrink-0 ring-1 ring-hairline/10" }), _jsx("span", { className: cn("truncate text-[11px]", isBlueKill ? "text-cyan-300/90" : "text-rose-300/90"), children: killer?.riotIdGameName ?? "Unknown" }), _jsx("span", { className: "text-flash/15 text-[10px] shrink-0 tracking-widest", children: ">" }), _jsx("span", { className: cn("truncate text-[11px]", !isBlueKill ? "text-cyan-300/90" : "text-rose-300/90"), children: victim?.riotIdGameName ?? "Unknown" }), victim && _jsx("img", { src: `${cdnBaseUrl()}/img/champion/${victim.championName}.png`, className: "w-4 h-4 rounded-[3px] shrink-0 ring-1 ring-hairline/10" }), assists > 0 && _jsxs("span", { className: "text-flash/15 text-[9px] shrink-0 tracking-wider", children: ["+", assists] })] }));
    }
    function renderObjectiveRow(event) {
        const killer = participantMap.get(event.killerId);
        const isBlue = event.killerTeamId === 100;
        const monsterName = getMonsterDisplayName(event.monsterType, event.monsterSubType);
        return (_jsxs("div", { className: "flex items-center gap-2 min-w-0", children: [killer && _jsx("img", { src: `${cdnBaseUrl()}/img/champion/${killer.championName}.png`, className: "w-4 h-4 rounded-[3px] shrink-0 ring-1 ring-hairline/10" }), _jsx("span", { className: cn("truncate text-[11px]", isBlue ? "text-cyan-300/90" : "text-rose-300/90"), children: killer?.riotIdGameName ?? (isBlue ? "Blue" : "Red") }), _jsx("span", { className: "text-flash/15 text-[10px] shrink-0 tracking-widest", children: ">" }), _jsx("span", { className: cn("shrink-0 text-[11px]", getMonsterColor(event.monsterType)), children: monsterName })] }));
    }
