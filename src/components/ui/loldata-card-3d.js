...
function bracket(ctx, x, y, dx, dy, arm = 88) {
    ctx.beginPath();
...
function paintWordmark(ctx, x, baseline, size, mainFill, dotFill, embossShadow = false) {
...
function paintBase(ctx, seed) {
    ...
    // vignette
    const vg = ctx.createRadialGradient(TEX_W / 2, TEX_H / 2, TEX_H * 0.35, TEX_W / 2, TEX_H / 2, TEX_W * 0.72);
...
function paintFrame(ctx, hairlineAlpha, bracketAlpha, corners) {
...
function paintSlots(ctx, digits, mode) {
...
function paintCredits(ctx, credits, mode) {
...
// ─── Face painters ───────────────────────────────────────────────────
function drawFront(ctx, credits, digits) {
...
function drawFrontRough(ctx, credits, digits) {
...
function drawFrontEmissive(ctx, credits, digits) {
...
function drawBack(ctx) {
    ...
    const hex = Array.from({ length: 56 }, () => Math.floor(rnd() * 256)
        .toString(16)
        .padStart(2, "0")
        .toUpperCase()).join(" ");
    ctx.font = `500 34px ${MONO}`;
...
function makeTexture(draw, srgb) {
...
function useCardTextures(credits, digits) {
    ...
    const textures = React.useMemo(() => {
        ...
        return {
            ...
            frontEmissive: makeTexture((ctx) => drawFrontEmissive(ctx, credits, d4), true),
...
function CardScene({ credits, digits, drag, spinSpeed, bobAmplitude, bobSpeed, baseTiltX, baseTiltZ, }) {
    ...
    useFrame((state, delta) => {
        ...
        if (d.active) {
            ...
            spin.rotation.x = THREE.MathUtils.clamp(spin.rotation.x + d.dy * TILT_PER_PX, -MAX_TILT, MAX_TILT);
            d.vy = THREE.MathUtils.clamp(dYaw / Math.max(delta, 1e-4), -MAX_FLING, MAX_FLING);
            d.dx = 0;
            d.dy = 0;
        }
        else {
    ...
    _jsx("group", { rotation: [baseTiltX, 0, baseTiltZ], children: _jsxs("group", { ref: spinRef, children: [_jsx("mesh", { geometry: bodyGeom, material: bodyMats }), _jsxs("mesh", { position: [0, 0, FACE_Z], children: [_jsx("planeGeometry", { args: [CARD_W, CARD_H] }), _jsx("meshPhysicalMaterial", { map: tex.front, roughnessMap: tex.frontRough, roughness: 1, bumpMap: tex.frontBump, bumpScale: 0.35, emissiveMap: tex.frontEmissive, emissive: "#ffffff", emissiveIntensity: 1.1, metalness: 0.92, envMapIntensity: 2.0, clearcoat: 0.5, clearcoatRoughness: 0.35, transparent: true })] }), _jsxs("mesh", { position: [0, 0, -FACE_Z], rotation: [0, Math.PI, 0], children: [_jsx("planeGeometry", { args: [CARD_W, CARD_H] }), _jsx("meshPhysicalMaterial", { map: tex.back, roughness: 0.34, emissiveMap: tex.backEmissive, emissive: "#ffffff", emissiveIntensity: 0.9, metalness: 0.9, envMapIntensity: 1.8, clearcoat: 0.4, clearcoatRoughness: 0.35, transparent: true })] })] }) }));
}
export function LoldataCard3D({ className, height = 380, credits = 150, digits = "4214", spinSpeed = 0.38, bobAmplitude = 0.06, bobSpeed = 0.85, baseTiltX = -0.2, baseTiltZ = -0.05, margin = 1.12, }) {
    ...
    return (_jsx("div", { className: className, style: {
            ...
            touchAction: "none",
        }, onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: endDrag, onPointerCancel: endDrag, children: _jsxs(Canvas, { camera: { position: [0, 0, 5], fov: 32 }, dpr: [1, 2], gl: {
                antialias: true,
                alpha: true,
                toneMapping: THREE.ACESFilmicToneMapping,
                outputColorSpace: THREE.SRGBColorSpace,
            }, children: [_jsx("ambientLight", { intensity: 0.28 }), _jsx("directionalLight", { position: [4, 5, 4], intensity: 1.4, color: "#ffffff" }), _jsx("directionalLight", { position: [-4, -2, 2], intensity: 0.45, color: "#00d992" }), _jsx("directionalLight", { position: [3, 2, -3], intensity: 0.32, color: "#ffb615" }), _jsxs(React.Suspense, { fallback: null, children: [_jsx(Bounds, { fit: true, clip: true, margin: margin, children: _jsx(CardScene, { credits: credits == null ? "—" : String(credits), digits: digits, drag: drag, spinSpeed: spinSpeed, bobAmplitude: bobAmplitude, bobSpeed: bobSpeed, baseTiltX: baseTiltX, baseTiltZ: baseTiltZ }) }), _jsx(Environment, { preset: "city" })] })] }) }));
}
