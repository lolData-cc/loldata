...
export const Jax = () => {
    ...
    React.useEffect(() => {
        if (!showPricing)
            return;
        const id = setTimeout(() => ref.current?.scrollIntoView({ behavior: "smooth", block: "center" }), 260);
    ...
    return (_jsx(motion.div, { ref: ref, 
        ...
        className: "relative bg-transparent w-screen left-1/2 -translate-x-1/2 overflow-hidden", initial: { opacity: 0, height: JAX_HEIGHT }, animate: inView
            ? { opacity: 1, height: showPricing ? PRICING_HEIGHT : JAX_HEIGHT }
            : { opacity: 0, height: JAX_HEIGHT }, transition: { duration: 0.5, ease: EASE_BRAND }, children: _jsxs(AnimatePresence, { initial: false, children: [!showPricing && (_jsx(motion.div, { 
                    // Pinned to the top 400px so it fades out IN PLACE — it never drifts
                    // down as the panel grows.
                    className: "absolute inset-x-0 top-0 h-[400px] w-full", initial: { opacity: 1 }, animate: { opacity: 1 }, exit: { opacity: 0 }, transition: { duration: 0.3, ease: EASE_BRAND }, children: _jsx(JaxCta, { inView: inView, reduceMotion: reduceMotion, onBecomeMember: () => setShowPricing(true) }) }, "jax"
                // Pinned to the top 400px so it fades out IN PLACE — it never drifts
                // down as the panel grows.
                )), showPricing && (_jsx(motion.div, { className: "absolute inset-0 w-full", initial: { opacity: 0, y: 12 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: 12 }, transition: { duration: 0.45, ease: EASE_BRAND, delay: 0.1 }, children: _jsx(PricingPanel, {}) }, "pricing"))] }) }));
};
...
function JaxCta({ inView, reduceMotion, onBecomeMember, }) {
    ...
    return (_jsxs("div", { className: "relative w-full h-full overflow-hidden", children: [_jsx("div", { "aria-hidden": true, className: "absolute inset-0 z-0 pointer-events-none opacity-[0.06] [background-image:radial-gradient(rgba(0,217,146,0.7)_1px,transparent_1px)] [background-size:24px_24px] [mask-image:radial-gradient(ellipse_at_24%_60%,black_5%,transparent_68%)] [-webkit-mask-image:radial-gradient(ellipse_at_24%_60%,black_5%,transparent_68%)]" }), _jsx(motion.div, { "aria-hidden": true, className: "absolute inset-0 z-0 pointer-events-none", style: {
                    background: "radial-gradient(ellipse 38% 55% at 22% 64%, rgba(0,217,146,0.10) 0%, transparent 72%)",
                }, animate: reduceMotion ? { opacity: 0.55 } : { opacity: [0.4, 0.62, 0.4] }, transition: reduceMotion ? undefined : { duration: 8, ease: "easeInOut", repeat: Infinity } }), _jsx(motion.img, { className: "hidden md:block absolute left-[20%] bottom-0 -translate-x-1/2 h-full w-[40%] object-contain object-bottom z-[1] pointer-events-none select-none [mask-image:linear-gradient(to_bottom,#000_85%,transparent)] [-webkit-mask-image:linear-gradient(to_bottom,#000_85%,transparent)]", alt: "", "aria-hidden": true, src: "/img/areuwithus_2.png", draggable: false, initial: { opacity: 0, x: -20 }, animate: inView ? { opacity: 0.96, x: 0 } : { opacity: 0, x: -20 }, transition: { duration: 0.9, ease: EASE_BRAND, delay: 0.1 }, style: {
...
function PricingPanel() {
    ...
    _jsx("div", { className: "flex h-full w-full flex-col overflow-y-auto no-scrollbar", children: _jsx("div", { className: "m-auto w-full", children: _jsx(PricingPlans, {}) }) }));
}
