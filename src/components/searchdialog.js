...
export function SearchDialog({ open, onOpenChange }) {
    ...
    const fetchAutocomplete = useCallback(async (query) => {
        ...
        const [apiRes, proRes] = await Promise.all([
            ...
            fetch(`${BOX_API_BASE_URL}/api/pros/search?query=${encodeURIComponent(searchTerm)}`, { signal: controller.signal })
                ...
                .catch((err) => {
                if (err.name !== "AbortError")
                    console.error("Pro search fetch:", err);
                return [];
            }),
        ]);
        ...
        const apiFiltered = (apiRes.results ?? [])
            .filter((s) => !proKeys.has(`${s.name}#${s.tag}`.toLowerCase()))
            .map((s) => ({
    ...
    return (_jsxs(Dialog, { open: open, onOpenChange: onOpenChange, children: [_jsx("div", { className: "hidden md:block", children: _jsx(DialogTrigger, { asChild: true, children: _jsx("div", { className: "font-jetbrains bg-jade/10 text-jade hover:bg-jade/20 items-center py-2 h-full px-3 rounded-sm cursor-clicker", children: "SEARCH A PLAYER" }) }) }), _jsx("div", { className: "md:hidden", children: _jsx(DialogTrigger, { asChild: true, children: _jsx("div", { className: "p-1.5 bg-jade/20 text-jade rounded-sm cursor-clicker", children: _jsx(Search, { className: "w-3 h-3" }) }) }) }), _jsx(DialogContent, { className: cn("sd-dialog w-[95vw] max-w-[500px] bg-transparent shadow-none border-none p-0", "top-[9vh] translate-y-0", "[&>button]:hidden"), children: _jsxs("div", { className: "relative w-full", children: [_jsxs(motion.div, { initial: {
                                ...
                                ease: [0.4, 0, 0.2, 1],
                                        ...
                                        times: [0, 0.32, 0.85],
                                        ease: "easeOut",
                                    } }), _jsx(BorderBeam, { duration: 8, size: 140 }), _jsxs(motion.div, { className: "relative z-10 px-5 py-7", initial: { opacity: 0 }, animate: { opacity: 1 }, transition: { duration: 0.22, delay: 0.42, ease: "easeOut" }, children: [_jsx(DialogTitle, { className: "sr-only", children: "Search a player" }), _jsxs("div", { className: "flex items-center gap-3 mb-4", children: [_jsx("span", { className: "w-[2px] h-3 bg-jade/70 shrink-0" }), _jsx("span", { className: "text-[10px] font-jetbrains text-flash/40 tracking-[0.24em] uppercase whitespace-nowrap", children: "Player Search" }), _jsx("span", { className: "flex-1 h-[1px] bg-gradient-to-r from-jade/14 to-transparent" }), _jsx(SavedProfiles, { onClick: showSavedProfilesList })] }), _jsxs("form", { onSubmit: (e) => {
                                                e.preventDefault();
                                                handleSubmit();
                                            }, 
                                            // Input and region share one surface: two boxes with a gap read
                                            // as a form, one field reads as an instrument.
                                            className: "sd-field flex items-stretch rounded-md bg-filmlight/[0.035] overflow-hidden", children: [_jsxs("label", { className: "relative flex-1", children: [_jsx("span", { className: "sd-tick", "aria-hidden": true }), _jsx(Search, { className: "absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-flash/25 pointer-events-none" }), _jsx("input", { ref: inputRef, value: input, onChange: (e) => {
                                                            ...
                                                            }, onKeyDown: handleKeyDown, placeholder: "Username#TAG", spellCheck: false, autoComplete: "off", 
                                                            // Chrome now lives on the shared .sd-field wrapper — the
                                                            // input itself is just transparent type on it.
                                                            className: cn("w-full h-12 bg-transparent border-0", "pl-10 pr-3 text-[14px] font-jetbrains text-flash", "placeholder:text-flash/20", "focus:outline-none focus:ring-0") })] }), _jsx("span", { "aria-hidden": true, className: "my-2.5 w-[1px] shrink-0 bg-jade/15" }), _jsxs(DropdownMenu, { children: [_jsxs(DropdownMenuTrigger
                                                        // Quiet by default: a filled jade chip fought the input for
                                                        // attention when the input is the thing you came here to use.
                                                        , { 
                                                            // Quiet by default: a filled jade chip fought the input for
                                                            // attention when the input is the thing you came here to use.
                                                            className: cn("group relative h-12 pl-4 pr-3.5 cursor-clicker outline-none shrink-0", "font-chakrapetch text-[11px] tracking-[0.18em] uppercase", "flex items-center gap-1.5", "text-flash/55 hover:text-jade transition-colors duration-200", "data-[state=open]:text-jade"), children: [_jsx("span", { "aria-hidden": true, className: cn("absolute inset-y-2.5 left-1 right-1 rounded-[4px] transition-colors duration-200", "group-hover:bg-jade/[0.09] group-data-[state=open]:bg-jade/[0.13]") }), _jsx("span", { className: "relative", children: region }), _jsx(ChevronDown, { className: "relative w-3 h-3 opacity-70 transition-transform duration-200 group-data-[state=open]:rotate-180" })] }), _jsx(DropdownMenuContent, { align: "end", sideOffset: 6, className: cn("min-w-[128px] p-1 rounded-md border border-jade/20", "bg-black/88 backdrop-blur-xl saturate-150", "shadow-[0_18px_48px_rgba(var(--c-shadow),0.75),0_0_24px_rgba(0,217,146,0.07)]"), children: REGIONS.map((r) => {
                                                                const active = r === region;
                                                                return (_jsx(DropdownMenuItem, { onClick: () => setRegion(r), 
                                                                    // A left tick marks the current region, so the choice
                                                                    // is readable without relying on the tint alone.
                                                                    className: cn("cursor-clicker rounded-[3px] pl-3 pr-3 py-1.5 border-l-2", "font-chakrapetch text-[11px] tracking-[0.18em] uppercase", "focus:bg-jade/10 focus:text-jade", "transition-colors duration-150", active
                                                                        ? "text-jade bg-jade/[0.08] border-jade"
                                                                        : "text-flash/50 border-transparent hover:border-jade/30"), children: r }, r));
                                                            }) })] })] }), _jsx(AnimatePresence, { initial: false, children: panelState && (_jsx(motion.div, { initial: { opacity: 0, height: 0 }, animate: { opacity: 1, height: "auto" }, exit: { opacity: 0, height: 0 }, transition: { duration: 0.22, ease: EASE_OUT }, className: "overflow-hidden", children: _jsxs("div", { className: "mt-3.5 pt-3 border-t border-jade/10 flex items-center justify-between gap-3", children: [_jsx("span", { className: cn("font-jetbrains text-[9px] tracking-[0.2em] lowercase", panelState === "loading" && "text-flash/35", panelState === "results" && "text-jade/70", panelState === "empty" && "text-flash/35"), children: panelState === "loading"
                                                                ? "searching"
                                                                : panelState === "results"
                                                                    ? `${displaySuggestions.length} ${displaySuggestions.length === 1 ? "result" : "results"}`
                                                                    : "no result" }), panelState === "results" && (_jsx("span", { className: "font-jetbrains text-[9px] tracking-[0.18em] lowercase text-flash/20 hidden sm:inline", children: "\u21B5 to open" }))] }) }, "status")) })] })] }), _jsx(AnimatePresence, { children: showPanel && (_jsxs(motion.div, { initial: { opacity: 0, y: -6 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -6 }, transition: { duration: 0.2, ease: EASE_OUT }, className: "absolute left-0 right-0 top-full mt-2 flex flex-col gap-1.5", children: [panelState === "loading" && _jsx(RowLoading, {}), panelState === "empty" && (_jsx(RowEmpty, { label: showingSaved ? "No saved profiles in this region" : "No player found" })), panelState === "results" &&
...
function RowShell({ children, className, }) {
    return (_jsx("div", { className: cn("relative overflow-hidden rounded-md", "bg-black/75 backdrop-blur-xl saturate-150", "shadow-[0_16px_44px_rgba(var(--c-shadow),0.65),inset_0_0_0_0.5px_rgba(255,255,255,0.08)]", className), children: children }));
}
function RowLoading() {
...
function SuggestionRow({ sugg, highlighted, saved, onHover, onPick, onToggleSave, }) {
    ...
    return (_jsxs("div", { onMouseEnter: onHover, onClick: onPick, className: cn("group relative overflow-hidden rounded-md cursor-clicker", "bg-black/75 backdrop-blur-xl saturate-150", "border transition-all duration-200", 
        ...
        isRecent
            ...
            boxShadow: isRecent
                ...
                : "0 16px 44px rgba(0,0,0,0.65), inset 0 0 0 0.5px rgba(255,255,255,0.08)",
        }, children: [_jsx("span", { "aria-hidden": true, className: cn("absolute left-0 top-2 bottom-2 w-[2px] rounded-full transition-opacity duration-200", highlighted ? "bg-jade opacity-100" : isRecent ? "bg-[#a78bfa] opacity-100" : "bg-jade opacity-0") }), _jsxs("div", { className: "flex items-center gap-3 px-4 py-3", children: [sugg._avatar ? (_jsx("img", { src: sugg._avatar, alt: "", className: "w-9 h-9 rounded-sm object-cover border border-jade/25 shrink-0" })) : sugg._isPro ? (_jsx("div", { className: "w-9 h-9 rounded-sm bg-filmdark/40 border border-jade/15 flex items-center justify-center shrink-0", children: _jsxs("svg", { viewBox: "0 0 64 52", className: "w-5 h-4", children: [_jsx("circle", { cx: "32", cy: "16", r: "9", fill: "rgba(0,217,146,0.15)", stroke: "rgba(0,217,146,0.25)", strokeWidth: "1" }), _jsx("path", { d: "M16 48c0-8.8 7.2-16 16-16s16 7.2 16 16", fill: "rgba(0,217,146,0.1)", stroke: "rgba(0,217,146,0.2)", strokeWidth: "1" })] }) })) : sugg.icon_id != null ? (_jsx("img", { 
                        ...
                        src: `${cdnBaseUrl()}/img/profileicon/${sugg.icon_id}.png`, alt: "", className: "w-9 h-9 rounded-sm border border-hairline/[0.06] shrink-0" })) : (_jsx("div", { className: "w-9 h-9 rounded-sm bg-filmlight/[0.03] border border-hairline/[0.06] flex items-center justify-center shrink-0", children: _jsx("span", { className: "text-[13px] font-chakrapetch font-bold text-flash/55", children: sugg.name.slice(0, 1).toUpperCase() }) })), _jsxs("div", { className: "flex flex-col gap-0.5 min-w-0 flex-1", children: [_jsxs("div", { className: "flex items-center gap-1.5 min-w-0", children: [_jsx("span", { className: cn("font-chakrapetch font-bold text-[14px] tracking-wide truncate transition-colors duration-200", highlighted ? "text-jade" : "text-flash group-hover:text-jade"), children: sugg._isPro && sugg._nickname ? sugg._nickname : sugg.name }), sugg._isPro && (_jsx("span", { className: "text-[7.5px] font-black px-1 py-[1px] rounded-[2px] tracking-wider shrink-0", style: {
                                            background: "linear-gradient(135deg, #00d992, #00b8ff)",
                                            color: "#040A0C",
                                        }, children: "PRO" })), sugg._plan === "elite" || sugg._plan === "premium" ? (_jsx("img", { src: `https://cdn2.loldata.cc/img/badge/loldata-plans/${sugg._plan === "elite" ? "Elite" : "Premium"}.png`, alt: sugg._plan === "elite" ? "Elite" : "Premium", title: sugg._plan === "elite" ? "Elite member" : "Premium member", className: "h-[18px] w-auto shrink-0 object-contain", draggable: false, onError: (e) => { e.target.style.display = "none"; } })) : null, isRecent && (_jsx("span", { className: "text-[8px] font-jetbrains font-bold uppercase px-1.5 py-[1px] rounded-[2px] tracking-[0.18em] shrink-0", style: {
