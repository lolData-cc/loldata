...
export function useScoutChat({ slug, activeTab, }) {
    ...
    const appendMessage = useCallback((m, fromSocket) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await fetch(`${API_BASE_URL}/api/scout/lobby/${slug}/chat?limit=100`);
                if (!res.ok || cancelled)
                    return;
    ...
    const send = useCallback(async (raw) => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/api/scout/lobby/${slug}/chat`, {
                method: "POST",
                headers: {
