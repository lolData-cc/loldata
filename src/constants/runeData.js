...
import { RUNE_TREES as STATIC_TREES } from "./rune-tree-data";
...
const stripPrefix = (icon) => icon.replace(/^perk-images\//, "");
function mapsFromTrees(trees) {
    const icon = new Map();
    const name = new Map();
...
export function getRuneName(id) {
    ensureRuneData();
    return _maps.name.get(id) ?? null;
}
export function getRuneTree(id) {
    ensureRuneData();
