...
export function getRuneTree(treeId) {
    return RUNE_TREES.find(t => t.id === treeId);
}
export function getRuneIcon(runeId) {
    for (const tree of RUNE_TREES) {
        for (const ks of tree.keystones)
            if (ks.id === runeId)
                return `${PERK_CDN}/${ks.icon}`;
        for (const row of tree.rows)
            for (const r of row)
                if (r.id === runeId)
                    return `${PERK_CDN}/${r.icon}`;
    }
    return null;
}
export function getRuneName(runeId) {
    for (const tree of RUNE_TREES) {
        for (const ks of tree.keystones)
            if (ks.id === runeId)
                return ks.name;
