...
export async function getAccountByRiotId(name, tag) {
    const endpoint = `https://${REGION}.api.riotgames.com/riot/account/v1/accounts/by-riot-id/${encodeURIComponent(name)}/${encodeURIComponent(tag)}`;
...
export async function getMatchIdsByPuuid(puuid, count = 5) {
    const res = await fetch(`https://${REGION}.api.riotgames.com/lol/match/v5/matches/by-puuid/${puuid}/ids?count=${count}`, {
...
export async function getRankedDataBySummonerId(summonerId) {
    const RIOT_API_KEY = process.env.RIOT_API_KEY;
    if (!RIOT_API_KEY) {
...
export async function getMatchDetails(matchId) {
    const res = await fetch(`https://${REGION}.api.riotgames.com/lol/match/v5/matches/${matchId}`, {
        headers: { "X-Riot-Token": RIOT_API_KEY },
    });
    if (!res.ok) {
...
export async function getMatchesWithWin(puuid, count = 5) {
...
export async function getLiveGameByPuuid(puuid) {
    const RIOT_API_KEY = process.env.RIOT_API_KEY;
    if (!RIOT_API_KEY)
        throw new Error("Missing Riot API key");
    const liveRes = await fetch(`https://euw1.api.riotgames.com/lol/spectator/v5/active-games/by-summoner/${puuid}`, {
        headers: { "X-Riot-Token": RIOT_API_KEY },
    });
    if (!liveRes.ok) {
