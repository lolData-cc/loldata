...
export async function howToWinHandler(req) {
    try {
        ...
        const champions = await Promise.all(enemyChampionIds.map((id) => getChampionData(id)));
        const summary = champions.map(c => `${c.name} (${c.type}, ${c.range_type}, roles: [${c.roles.join(", ")}], tags: [${(c.tags || []).join(", ")}])`).join("\n");
        const prompt = `
