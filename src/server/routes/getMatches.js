...
export async function getMatchesHandler(req) {
    try {
        ...
        for (const matchId of matchIds) {
            try {
                ...
                if (!participant)
                    continue;
                const { win, championName = "Unknown", goldEarned = 0, kills = 0, deaths = 0, assists = 0, totalMinionsKilled = 0, neutralMinionsKilled = 0, } = participant;
        ...
        championsWithSortKey.sort((a, b) => {
            if (b.sortGames !== a.sortGames)
                return b.sortGames - a.sortGames;
            if (b.sortWinrate !== a.sortWinrate)
                return b.sortWinrate - a.sortWinrate;
