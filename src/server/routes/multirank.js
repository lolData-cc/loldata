...
export async function getMultiRankHandler(req) {
    try {
        ...
        const results = await Promise.all(riotIds.map(async (fullRiotId) => {
            ...
            try {
                ...
                const solo = rankedData.find((entry) => entry.queueType === "RANKED_SOLO_5x5");
                return {
                    riotId: fullRiotId,
