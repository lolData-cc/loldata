export function calculateLolDataScores(participants) {
    ...
    const objRaw = participants.map(p => (p.challenges?.turretTakedowns ?? 0) +
        (p.challenges?.dragonTakedowns ?? 0) +
        (p.challenges?.baronTakedowns ?? 0));
    // --- Team-wise normalization ---
    const kda_n = normalizeTeam(kdaRobust, idxTeam100).map((_, i) => i in kdaRobust ? normalizeTeam(kdaRobust, idxTeam200)[i] ?? _ : _);
    const vision_n = normalizeTeam(visionSpm, idxTeam100).map((_, i) => i in visionSpm ? normalizeTeam(visionSpm, idxTeam200)[i] ?? _ : _);
    const obj_n = normalizeTeam(objRaw, idxTeam100).map((_, i) => i in objRaw ? normalizeTeam(objRaw, idxTeam200)[i] ?? _ : _);
    ...
    const isTopNInTeam = (metric, idxs, n = 2) => {
    ...
    const pickWeights = (i) => (supportFlag[i] ? W_SUPPORT : (frontlineFlag[i] ? W_FRONTLINE : W_CARRY));
    ...
    const mvpForTeam = (teamId) => {
        ...
        return (pool.length ? pool : teamPlayers).reduce((best, p) => (scores[p.puuid] > (scores[best] ?? -1) ? p.puuid : best), base);
    };
    const mvpWin = mvpForTeam(teams.winId);
    const mvpLose = mvpForTeam(teams.loseId);
    return { scores, mvpWin, mvpLose };
}
export function calculatePlayerRating(matches, puuid, maxGames = 15) {
    ...
    const deathPenalty = deathsOver > 0 ? clamp(deathsOver / 5, 0, 1) * 10 : 0; // max -10
    // --- COMBINAZIONE FINALE ---
    let rating = 40 + // base: non scendi mai sotto 40
        winrateScore +
        mvpScore +
        kdaScore +
        killsScore -
