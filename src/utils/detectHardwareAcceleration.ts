...
function checkWebGLRenderer(): "software" | "hardware" | "unknown" {
  try {
    ...
    if (!gl) return "software"; // no WebGL at all → essentially unaccelerated

    const debugInfo = gl.getExtension("WEBGL_debug_renderer_info");
    if (!debugInfo) return "unknown"; // browser masks renderer (Firefox by default)
...
function measureFps(frames: number, timeoutMs: number): Promise<number> {
...
export async function detectHardwareAcceleration(): Promise<boolean | null> {
  ...
  if (rendererResult === "hardware") return true;
