...
const glassCard = cn("relative overflow-hidden rounded-md", "bg-filmdark/25 backdrop-blur-lg saturate-150 glass-panel", "shadow-[0_10px_30px_rgba(var(--c-shadow),0.55),inset_0_0_0_0.5px_rgba(255,255,255,0.10),inset_0_1px_0_rgba(255,255,255,0.05)]");
// ── Section Header ──────────────────────────────────────────────────
...
export default function MatchPage() {
    ...
    function StatBar({ pct, avgPct, isAboveAvg, delay }) {
        ...
        return (_jsxs("div", { className: "relative h-[22px] w-full rounded-[3px] bg-filmlight/[0.04] border border-hairline/[0.04] overflow-hidden", children: [_jsx("div", { className: "absolute inset-y-0 left-0 rounded-[2px]", style: {
                        width: `${w}%`,
    ...
    const renderItems = (p) => (_jsx("div", { className: "flex gap-0.5", children: Array.from({ length: 7 }, (_, i) => {
            const id = p[`item${i}`];
            return id > 0 ? (_jsx("img", { src: `${cdnBaseUrl()}/img/item/${id}.png`, className: "w-5 h-5 rounded-[3px] ring-1 ring-hairline/10" }, i)) : (_jsx("div", { className: "w-5 h-5 rounded-[3px] bg-filmlight/[0.03] ring-1 ring-hairline/[0.06]" }, i));
        }) }));
    ...
    function PlayerRow({ p, team, side }) {
        ...
        return (_jsxs("div", { className: cn("flex items-center gap-3 px-3 py-2 transition-all", "border-b border-hairline/[0.04] last:border-b-0", "hover:bg-filmlight/[0.02]", isFocused && "bg-jade/[0.04]"), children: [_jsxs("div", { className: "flex items-center gap-1.5 shrink-0", children: [_jsxs("div", { className: "relative w-8 h-8", children: [_jsx("img", { src: `${cdnBaseUrl()}/img/champion/${p.championName}.png`, className: "w-8 h-8 rounded-[4px] ring-1 ring-hairline/10" }), (isMvp || isAce) && (_jsx("span", { className: cn("absolute -top-1 -right-1 text-[7px] px-0.5 rounded-sm z-10 font-mono font-bold", isMvp && "bg-pine text-jade", isAce && "bg-[#3A2C45] text-[#C693F1]"), style: { lineHeight: '1' }, children: isMvp ? "MVP" : "ACE" })), p.champLevel && (_jsx("span", { className: "absolute -bottom-0.5 -right-0.5 text-[8px] bg-black/90 text-flash/50 rounded px-0.5 leading-none font-mono", children: p.champLevel }))] }), _jsxs("div", { className: "flex flex-col gap-0.5", children: [_jsx("img", { src: summonerSpellUrl(p.summoner1Id), className: "w-4 h-4 rounded-[2px]" }), _jsx("img", { src: summonerSpellUrl(p.summoner2Id), className: "w-4 h-4 rounded-[2px]" })] }), p.perks?.styles && p.perks.styles.length >= 2 && (_jsxs("div", { className: "flex flex-col gap-0.5", children: [(() => {
                            ...
                            return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex items-center gap-1", children: [p.riotIdGameName && p.riotIdTagline ? (_jsx(Link, { to: `/summoners/${region}/${(p.riotIdGameName || "").replace(/\s+/g, "+")}-${p.riotIdTagline}`, className: cn("truncate text-[11px] font-mono tracking-wide hover:underline cursor-clicker", isFocused ? "text-jade font-bold" : teamColor), onClick: (e) => {
                                                ...
                                                }, children: isPro ? "PRO" : "STR" }))] }), identity && (_jsx(Link, { to: `/players/${identity.slug}`, onClick: (e) => e.stopPropagation(), title: `${identity.name} — profile`, className: cn("block truncate text-[10px] font-chakrapetch font-semibold leading-none mt-[3px] transition-colors cursor-clicker", isPro
                                            ? "text-jade/70 hover:text-jade"
                                            : "text-[#c9a3e0]/70 hover:text-[#c9a3e0]"), children: identity.name }))] }));
                        })(), (() => {
    ...
    return (_jsxs("div", { className: "text-flash", children: [_jsxs("div", { className: "relative w-full h-[380px] overflow-hidden", children: [mvpPlayer && (_jsx("img", { src: cdnSplashUrl(mvpPlayer.championName), alt: mvpPlayer.championName, className: "absolute inset-0 w-full h-full object-cover", style: { objectPosition: `center ${splashPositionMap[mvpPlayer.championName] || "15%"}` }, draggable: false })), _jsx("div", { className: "absolute inset-0 bg-liquirice/70" }), _jsx("div", { className: "absolute inset-0 pointer-events-none opacity-[0.04] z-[2]", style: {
                            backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(255,255,255,0.15) 2px, rgba(255,255,255,0.15) 4px)",
                                                ...
                                                return (_jsxs(TabsTrigger, { value: p.puuid, className: cn("flex items-center gap-2 text-left px-2 py-1.5 rounded-[4px] w-full justify-start transition-all cursor-clicker", "font-mono text-[10px] tracking-wide", "text-flash/35 hover:text-flash/60 hover:bg-filmlight/[0.03]", "data-[state=active]:bg-filmlight/[0.06] data-[state=active]:text-flash/90", "data-[state=active]:shadow-[inset_0_0_12px_rgba(0,217,146,0.04)]"), children: [_jsx("img", { src: `${cdnBaseUrl()}/img/champion/${p.championName}.png`, className: "w-6 h-6 rounded-[3px] ring-1 ring-hairline/10 shrink-0" }), _jsx("span", { className: cn("truncate", p.puuid === focusedPlayerPuuid && "text-jade font-bold"), children: p.riotIdGameName ?? "Unknown" }), (() => {
                                                            ...
                                                            return (_jsxs(_Fragment, { children: [_jsx("span", { className: "shrink-0 text-[7px] font-black leading-none px-[2px] py-[1px] rounded-[2px] tracking-wide", style: {
                                                                            background: isPro
                                                                                ? "linear-gradient(135deg, #00d992, #00b8ff)"
                                                                                : "linear-gradient(135deg, #7b42a1, #a855c7)",
                                                                            color: isPro ? "#040A0C" : "#e0d0f0",
                                                                        }, children: isPro ? "PRO" : "STR" }), identity && (_jsx("span", { className: cn("shrink-0 max-w-[64px] truncate text-[9px] font-chakrapetch font-semibold leading-none", isPro ? "text-jade/70" : "text-[#c9a3e0]/70"), children: identity.name }))] }));
