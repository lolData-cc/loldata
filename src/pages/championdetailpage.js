...
import { championSeo, championPath, TAB_LABEL, CHAMPION_TABS } from "@/lib/championSeo";
...
const ROLE_SHORT = { TOP: "TOP", JUNGLE: "JGL", MIDDLE: "MID", BOTTOM: "ADC", UTILITY: "SUP" };
function HeroTier({ tier, role }) {
    const c = tierColor(tier);
    return (_jsxs("div", { className: "flex flex-col", title: role ? `Current meta tier in ${role}` : "Current meta tier", children: [_jsx("span", { className: "font-chakrapetch text-[20px] sm:text-[24px] font-bold leading-none", style: { color: c, textShadow: tier ? `0 0 24px ${c}59` : undefined }, children: tier ? `${tier}` : "—" }), _jsx("span", { className: "mt-1 font-jetbrains text-[9px] uppercase tracking-[0.18em] text-flash/40", children: role && ROLE_SHORT[role] ? `Tier · ${ROLE_SHORT[role]}` : "Tier" })] }));
}
const validTabs = ["overview", "build", "duos", "counters", "guides", "pros"];
export default function ChampionDetailPage() {
    ...
    const opponentIcon = (k) => `${cdnBaseUrl()}/img/champion/${opponentIdFromKey(k)}.png`;
    //retrieve matchup
    useEffect(() => {
        let cancelled = false;
    ...
    useSeo({
        ...
        canonical: championPath(champId ?? "", activeTab),
    });
...
function Panel({ className, children }) {
    return (_jsx("div", { className: cn("relative overflow-hidden rounded-md bg-filmdark/25 backdrop-blur-lg saturate-150 glass-panel", "shadow-[0_10px_30px_rgba(var(--c-shadow),0.45),inset_0_0_0_0.5px_rgba(255,255,255,0.08),inset_0_1px_0_rgba(255,255,255,0.04)]", className), children: children }));
}
function StatChip({ label, value, tone = "jade" }) {
    return (_jsxs("span", { className: "inline-flex items-center gap-1.5 rounded-[4px] bg-filmdark/40 px-2 py-1 shadow-[inset_0_0_0_0.5px_rgba(255,255,255,0.08)]", children: [_jsx("span", { className: "font-jetbrains text-[9px] uppercase tracking-[0.14em] text-flash/40", children: label }), _jsx("span", { className: cn("font-jetbrains text-[11px] font-semibold tabular-nums", tone === "sky" ? "text-sky-300/85" : "text-jade/90"), children: value })] }));
}
function TipsPanel({ title, tips, tone }) {
    return (_jsxs(Panel, { className: "p-5", children: [_jsx(SectionTitle, { tone: tone === "enemy" ? "red" : "jade", children: title }), _jsx("ul", { className: "mt-3.5 space-y-2.5", children: tips.map((tip, i) => (_jsxs("li", { className: "flex gap-2.5 font-chakrapetch text-[13px] leading-relaxed text-flash/70", children: [_jsx("span", { className: cn("mt-px shrink-0 font-bold", tone === "ally" ? "text-jade" : "text-[#ff6286]"), children: "\u25B8" }), tip] }, i))) })] }));
}
function ChampOverview({ champ }) {
    ...
    const dedupe = (arr) => (arr ? [...new Set(arr)].join(" / ") : "");
    const iconUrl = (key, image) => key === "P" ? `${cdnBaseUrl()}/img/passive/${image}` : `${cdnBaseUrl()}/img/spell/${image}`;
    return (
    ...
    _jsxs("div", { className: "grid items-start gap-4 lg:grid-cols-12", children: [_jsxs("div", { className: "flex flex-col gap-4 lg:col-span-7", children: [abilities.length > 0 && (_jsxs(Panel, { className: "p-5", children: [_jsx(SectionTitle, { children: "Abilities" }), _jsxs("div", { className: "mt-4 flex items-center gap-2", children: [abilities.map((ab, idx) => {
                                        const on = idx === selectedAbility;
                                        return (_jsxs("button", { type: "button", onClick: () => setSelectedAbility(idx), className: "group relative cursor-clicker", "aria-label": ab.name, children: [_jsx("img", { src: iconUrl(ab.key, ab.image), alt: "", className: cn("h-12 w-12 rounded-md object-cover transition-all", on ? "ring-2 ring-jade shadow-[0_0_16px_rgba(0,217,146,0.4)]" : "opacity-60 ring-1 ring-hairline/10 group-hover:opacity-100 group-hover:ring-jade/40") }), _jsx("span", { className: cn("absolute -bottom-1 -right-1 rounded-[2px] px-1 text-[8px] font-black leading-[1.5]", on ? "bg-jade text-black" : "bg-black/80 text-flash/55"), children: ab.key })] }, ab.key));
                                    }), _jsxs("div", { className: "ml-auto hidden flex-wrap justify-end gap-2 sm:flex", children: [active.cooldown && active.cooldown.some((v) => v > 0) && _jsx(StatChip, { label: "CD", value: `${dedupe(active.cooldown)}s` }), active.cost && active.cost.some((v) => v > 0) && _jsx(StatChip, { label: "Cost", value: dedupe(active.cost), tone: "sky" })] })] }), _jsxs(motion.div, { initial: { opacity: 0, y: 6 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.25, ease: "easeOut" }, className: "mt-4 min-h-[112px] border-t border-hairline/[0.06] pt-3.5", children: [_jsxs("div", { className: "flex items-center gap-2.5", children: [_jsx("span", { className: "rounded-[3px] bg-jade px-1.5 py-0.5 font-jetbrains text-[10px] font-black text-black", children: active.key }), _jsx("h4", { className: "font-chakrapetch text-[15px] font-bold uppercase tracking-[0.02em] text-flash", children: active.name })] }), _jsx("p", { className: "mt-2.5 font-chakrapetch text-[13px] leading-[1.65] text-flash/75", children: cleanDesc(active.description) })] }, active.key)] })), champ.allytips && champ.allytips.length > 0 && _jsx(TipsPanel, { title: `Playing as ${champ.name}`, tips: champ.allytips, tone: "ally" }), champ.enemytips && champ.enemytips.length > 0 && _jsx(TipsPanel, { title: `Playing against ${champ.name}`, tips: champ.enemytips, tone: "enemy" })] }), _jsxs("div", { className: "flex flex-col gap-4 lg:col-span-5", children: [(champ.info || champ.stats) && (_jsxs(Panel, { className: "p-5", children: [_jsx(SectionTitle, { children: "Combat Profile" }), champ.info && (_jsx("div", { className: "mt-4 space-y-2.5", children: ["attack", "defense", "magic", "difficulty"].map((stat) => {
                                    const val = champ.info[stat];
                                    const citrine = stat === "difficulty";
                                    return (_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("span", { className: "w-[84px] shrink-0 font-jetbrains text-[9.5px] uppercase tracking-[0.18em] text-flash/40", children: stat }), _jsx("div", { className: "h-[5px] flex-1 overflow-hidden rounded-full bg-filmlight/[0.06]", children: _jsx(motion.div, { initial: { width: 0 }, animate: { width: `${val * 10}%` }, transition: { delay: 0.2, duration: 0.6, ease: "easeOut" }, className: cn("h-full rounded-full bg-gradient-to-r", citrine ? "from-citrine to-citrine/40" : "from-jade to-jade/40") }) }), _jsx("span", { className: cn("w-5 shrink-0 text-right font-chakrapetch text-[13px] font-bold tabular-nums", citrine ? "text-citrine" : "text-jade"), children: val })] }, stat));
                                }) })), champ.stats && (_jsxs(_Fragment, { children: [_jsxs("div", { className: "mb-3.5 mt-5 flex items-center gap-2.5", children: [_jsx("span", { className: "font-jetbrains text-[9px] uppercase tracking-[0.24em] text-flash/35", children: "Base stats" }), _jsx("div", { className: "h-px flex-1 bg-hairline/[0.06]" }), champ.partype && _jsx("span", { className: "font-jetbrains text-[9px] uppercase tracking-[0.14em] text-flash/45", children: champ.partype })] }), _jsx("div", { className: "grid grid-cols-1 gap-x-6 sm:grid-cols-2", children: STAT_PAIRS.filter((p) => champ.stats[p.key] !== undefined &&
                                            // manaless champs: hide the dead "Mana 0" / "MP Regen 0" rows
                                            !((p.key === "mp" || p.key === "mpregen") && !champ.stats[p.key])).map((p) => (_jsxs("div", { className: "flex items-baseline justify-between gap-2 border-b border-hairline/[0.05] py-[6px]", children: [_jsx("span", { className: "font-jetbrains text-[10.5px] uppercase tracking-[0.08em] text-flash/40", children: p.label }), _jsxs("span", { className: "font-jetbrains tabular-nums", children: [_jsx("span", { className: "text-[12.5px] font-semibold text-flash/85", children: champ.stats[p.key] }), p.per && champ.stats[p.per] ? _jsxs("span", { className: "ml-1 text-[10px] text-jade/55", children: ["+", champ.stats[p.per]] }) : null] })] }, p.key))) })] }))] })), champ.lore && (_jsxs(Panel, { className: "p-5", children: [_jsx(SectionTitle, { children: "Lore" }), _jsxs("div", { className: "relative mt-3 pl-6", children: [_jsx("span", { className: "absolute -top-3 left-0 select-none font-chakrapetch text-[44px] leading-none text-jade/20", children: "“" }), _jsx("p", { className: "overflow-hidden font-chakrapetch text-[13px] leading-[1.75] text-flash/70", style: !loreOpen && champ.lore.length > 420 ? { display: "-webkit-box", WebkitLineClamp: 7, WebkitBoxOrient: "vertical" } : undefined, children: champ.lore }), champ.lore.length > 420 && (_jsx("button", { type: "button", onClick: () => setLoreOpen((v) => !v), className: "mt-2.5 font-jetbrains text-[9px] uppercase tracking-[0.2em] text-jade/60 transition-colors hover:text-jade cursor-clicker", children: loreOpen ? "▴ Show less" : "▾ Read more" }))] })] }))] })] }));
}
function SectionTitle({ children, tone = "jade" }) {
    const red = tone === "red";
    const dot = red ? "#ff6286" : "#00d992";
    return (_jsxs("div", { className: "flex items-center gap-2.5", children: [_jsx("span", { className: "w-1.5 h-1.5 rounded-full shrink-0", style: { background: dot, boxShadow: `0 0 8px ${dot}` } }), _jsx("span", { className: cn("font-chakrapetch text-[11px] font-bold tracking-[0.28em] uppercase", red ? "text-[#ff6286]/85" : "text-jade/80"), children: children }), _jsx("div", { className: cn("h-px flex-1 bg-gradient-to-r to-transparent", red ? "from-[#ff6286]/20" : "from-jade/20") })] }));
}
