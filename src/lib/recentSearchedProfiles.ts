...
export type RecentSearchedProfile = {
  name: string
  tag: string
  region: string
  icon_id: number | null
 ...
 * close & reopen it.
 */
export function enrichRecentProfile(
  name: string,
  tag: string,
  region: string,
  ...
  const next = cur.map((r) => {
    ...
    const mergedIcon =
      r.icon_id == null && patch.icon_id != null ? patch.icon_id : r.icon_id
    const mergedRank =
...
export function removeRecentProfile(
  ...
  const next = cur.filter(
    (r) => `${r.name}#${r.tag}#${r.region}`.toLowerCase() !== targetKey
  )
