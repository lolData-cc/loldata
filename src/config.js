 ...
 */
export const cdnVersionReady = fetch(`${CDN_ORIGIN}/_current_version.txt`, { signal: AbortSignal.timeout(5_000) })
...
export function cdnSplashUrl(champName, skinNum = 0) {
...
export function summonerSpellUrl(spellId) {
...
export const API_BASE_URL = import.meta.env.VITE_API_URL ?? "https://api2.loldata.cc";
...
export const EXPLORER_API_BASE_URL = import.meta.env.VITE_EXPLORER_API_URL || "https://api2.loldata.cc";
...
export const BOX_API_BASE_URL = import.meta.env.VITE_EXPLORER_API_URL || "https://api2.loldata.cc";
...
export function normalizeChampName(name) {
    return ICON_NAME_FIXES[name] ?? name;
}
export function normalizeChampSplash(name) {
    return SPLASH_NAME_FIXES[name] ?? name;
}
export function champDisplayName(name) {
